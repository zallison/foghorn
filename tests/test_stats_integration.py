...
def test_stats_collected_on_query():
    ...
    assert snapshot.totals["total_queries"] >= 1, "Should have recorded at least 1 query"
    assert "A" in snapshot.qtypes, "Should have recorded query type"
    assert snapshot.totals.get("cache_misses", 0) >= 1, "Should have recorded cache miss"
