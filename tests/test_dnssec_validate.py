...
def test_validate_authority_rrsets_behaviour(monkeypatch):
    ...
    monkeypatch.setattr(
        "foghorn.dnssec_validate.dns.dnssec.validate", _boom_validate
    )
...
def test_classify_dnssec_local_extended_additional_paths(monkeypatch):
    ...
    for status in ("dnssec_secure", "dnssec_bogus", None):
        monkeypatch.setattr(
            dval, "_classify_dnssec_local", lambda *_a, **_k: status
        )
    ...
    monkeypatch.setattr(dval, "_classify_dnssec_local", lambda *_a, **_k: "dnssec_unsigned")
    monkeypatch.setattr(
        "foghorn.dnssec_validate.dns.message.from_wire", fake_from_wire
    )
    monkeypatch.setattr(dval, "_message_has_dnssec_rr", fake_has_dnssec_rr)
...
def test_configure_dnssec_resolver_and_resolver_variants(monkeypatch):
    ...
    class _FakeRecursiveResolver:
        ...
        def resolve(self, q):  # noqa: D401, ANN001
            ...
            msg.answer.append(
                dns.rrset.from_text(
                    "example.com.", 300, dns.rdataclass.IN, dns.rdatatype.A, "203.0.113.5"
                )
            )
            wire = msg.to_wire()
