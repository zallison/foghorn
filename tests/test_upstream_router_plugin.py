        ...
        routes=[
            {
                ...
                "upstream": {"host": "10.0.0.53", "port": 5353},
            }
        ]
    )
    ctx = PluginContext(client_ip="1.2.3.4")
    plugin.pre_resolve("corp.example.com", 1, b"", ctx)
...
def test_suffix_match_with_and_without_leading_dot():
    plugin = UpstreamRouterPlugin(
        routes=[
            {"suffix": "internal", "upstream": {"host": "192.168.1.1", "port": 53}},
            {
                ...
                "upstream": {"host": "127.0.0.1", "port": 1053},
            },
        ]
    )
    ...
    plugin.pre_resolve("KUBE-DNS.SVC.CLUSTER.LOCAL.", 1, b"", ctx3)
...
def test_no_match_leaves_override_none():
    plugin = UpstreamRouterPlugin(
        routes=[
            {
                "domain": "corp.example.com",
                "upstream": {"host": "10.0.0.53", "port": 53},
            }
        ]
    )
    ctx = PluginContext(client_ip="1.2.3.4")
    plugin.pre_resolve("example.com", 1, b"", ctx)
