...
def test_resolve_root_qname_no_servers_synthesizes_servfail(
    ...
    monkeypatch.setattr(RecursiveResolver, "_choose_initial_servers", fake_choose_initial_servers)
...
def test_resolve_counts_leading_underscore_labels(monkeypatch: pytest.MonkeyPatch) -> None:
...
def test_resolve_honours_overall_deadline(monkeypatch: pytest.MonkeyPatch) -> None:
    ...
    def _boom_udp(*_a: Any, **_k: Any) -> bytes:  # noqa: ANN001
        raise AssertionError("udp_query should not be reached when deadline is exceeded")
...
def test_resolve_visited_loop_guard_and_delegation_follow(monkeypatch: pytest.MonkeyPatch) -> None:
    ...
    monkeypatch.setattr(RecursiveResolver, "_extract_next_servers", fake_extract_next_servers)
...
def test_resolve_tcp_fallback_on_tc_in_main_loop(monkeypatch: pytest.MonkeyPatch) -> None:
...
def test_resolve_final_fallthrough_without_delegation(monkeypatch: pytest.MonkeyPatch) -> None:
